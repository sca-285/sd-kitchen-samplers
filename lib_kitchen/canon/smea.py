...
"""reForge's Euler Dy / SMEA Dy / Negative samplers (Koishi-Star's originals).

GENERATED by tools/vendor.py - do not edit by hand; edit the generator.

Source: reForge modules/sd_samplers_kdiffusion_smea.py at commit 739b2e1d9ab63160eaff9c8f73172c8da68424e1.
...
def sample_euler_dy(model, x, sigmas, extra_args=None, callback=None, disable=None):
    s_churn = _host.opt('euler_dy_s_churn', 0.0)
    s_tmin = _host.opt('euler_dy_s_tmin', 0.0)
    s_tmax = float('inf')
    s_noise = _host.opt('euler_dy_s_noise', 1.0)
...
def sample_euler_smea_dy(model, x, sigmas, extra_args=None, callback=None, disable=None):
    s_churn = _host.opt('euler_smea_dy_s_churn', 0.0)
    s_tmin = _host.opt('euler_smea_dy_s_tmin', 0.0)
    s_tmax = float('inf')
    s_noise = _host.opt('euler_smea_dy_s_noise', 1.0)
    extra_args = {} if extra_args is None else extra_args
    s_in = x.new_ones([x.shape[0]])
    for i in trange(len(sigmas) - 1, disable=disable):
        gamma = max(s_churn / (len(sigmas) - 1), 2 ** 0.5 - 1) if s_tmin <= sigmas[i] <= s_tmax else 0.
...
def sample_euler_negative(model, x, sigmas, extra_args=None, callback=None, disable=None):
    s_churn = _host.opt('euler_negative_s_churn', 0.0)
    s_tmin = _host.opt('euler_negative_s_tmin', 0.0)
    s_tmax = float('inf')
    s_noise = _host.opt('euler_negative_s_noise', 1.0)
    extra_args = {} if extra_args is None else extra_args
    s_in = x.new_ones([x.shape[0]])
    for i in trange(len(sigmas) - 1, disable=disable):
        # print(i)
...
def sample_euler_dy_negative(model, x, sigmas, extra_args=None, callback=None, disable=None):
    s_churn = _host.opt('euler_dy_negative_s_churn', 0.0)
    s_tmin = _host.opt('euler_dy_negative_s_tmin', 0.0)
    s_tmax = float('inf')
    s_noise = _host.opt('euler_dy_negative_s_noise', 1.0)
    extra_args = {} if extra_args is None else extra_args
    s_in = x.new_ones([x.shape[0]])
    for i in trange(len(sigmas) - 1, disable=disable):
        # print(i)
